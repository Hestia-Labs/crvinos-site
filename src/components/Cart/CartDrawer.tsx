...
import BasicButton from '@/components/Buttons/BasicButton'; // Importar BasicButton

const CartDrawer: React.FC = () => {
  const { isOpen, activeDrawer, closeDrawer } = useDrawer();
  const { cartItems, removeFromCart, updateCartItem, totalItems, totalPrice } = useCart();
                  ...
                  {cartItems.length > 0 ? (
                    cartItems.map(item => (
                      ...
                      >
                        ...
                        <button onClick={() => handleRemove(item.id)} className="text-crred underline hover:text-crred-75">Eliminar</button>
                      </motion.div>
                    ))
                  ) : (
